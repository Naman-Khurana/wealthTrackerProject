...
//(hasRole('PREMIUM_USER') || hasRole('ADMIN')) &&
@PreAuthorize("#userid.toString() == principal.username && (hasRole('PREMIUM_USER') || hasRole('ADMIN'))")
@RestController
...
public class StockMarketController {
    ...
    @GetMapping("/list")
    public ResponseEntity<List<StockPrice>> getStockList(@PathVariable("userid") int userid){
    ...
    public ResponseEntity<StockPrice> getCompanyStockInfo(@PathVariable("userid") int userid,@PathVariable("companyName") String companyName){
    ...
    public ResponseEntity<CompanyInfoDTO> getCompanyStockInfoByTradableSearchAPI(@PathVariable("userid") int userid,@PathVariable("companyName") String companyName){
        if(companyName==null){
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
