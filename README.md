...
| Method | Endpoint | Description |
|---|---|---|
| GET | `/` | Financial overview (expenses + earnings) |

### Expenses — `/api/{userid}/expenses`
| Method | Endpoint | Description |
|---|---|---|
| GET | `/` | List all expenses (supports date range filter) |
| POST | `/add` | Add a new expense |
| PUT | `/update` | Update an existing expense |
| DELETE | `/delete/{expenseid}` | Delete an expense |
| GET | `/{expenseid}` | Get expense by ID |
| GET | `/essential` | Total essential expenses |
| GET | `/luxury` | Total luxury expenses |

### Budget — `/api/{userid}/expenses/budget`
| Method | Endpoint | Description |
|---|---|---|
| POST | `/set` | Set a new budget |
| GET | `/get` | Get all budgets |
| PUT | `/update/{budgetId}` | Update a budget |

### Earnings — `/api/{userid}/earnings`
| Method | Endpoint | Description |
|---|---|---|
| GET | `/` | List all earnings (supports date range filter) |
| POST | `/add` | Add a new earning |
| PUT | `/update` | Update an existing earning |
| DELETE | `/delete/{earningid}` | Delete an earning |
| GET | `/{earningid}` | Get earning by ID |

### Stocks — `/api/{userid}/invest/stock`
| Method | Endpoint | Description |
|---|---|---|
| GET | `/list` | List available stocks (Premium/Admin only) |
| GET | `/{companyName}` | Get stock info for a company |
| GET | `/company/{companyName}` | Get detailed company info via FMP API |
